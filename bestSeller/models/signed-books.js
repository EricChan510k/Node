...
var select_sql =
  ` LTRIM(RTRIM(inventory.catalog)) AS catalog, 
    ...
    WHERE 
      bookstore_topx.sectionid = 4 
      AND publish = 1 
      AND (stock.discont = 0 AND (stock.units - stock.qtyreserved) > 0) 
      AND SUBSTRING(stock.number_binding, 1, 3) <> 'LTD' 
      AND inventory.ltd_editio = 0 
      AND SUBSTRING(number_binding, 4, 3) = 'SIG'`;

function getSignedBooks (skip, take, featured_book, callback) {
